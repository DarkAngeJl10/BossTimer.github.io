...
document.addEventListener("DOMContentLoaded", async () => {
    let response = await fetch(`${window.apidomain}/check_session.php`, {
...
function ChangeRoles() {
    // Просто перенаправляем на AdminPanel, проверка прав будет происходить на сервере
...
function connectWebSocket() {
    ...
    socket.addEventListener('message', function handleMessage(event) {
        try {
            ...
            // Если получен статус success от Permissions
            if (data.type === 'permissions' && data.status === 'success') {
                // Отправляем запрос на боссов
            ...
            if (data.status === 'success' && data.permissions) {
                // Если у пользователя есть права на управление ролями
                if (data.permissions.can_manage_roles) {
                    // Находим кнопку по ID
                    const manageRolesButton = document.getElementById('manageRolesButton');
                    // Отображаем кнопку
                    manageRolesButton.style.display = 'inline-block';
                }
            }
    
            // Обработка других типов сообщений
            if (data.action === 'onlineUsers') {
